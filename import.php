...
if( isset( $_FILES[ 'import_campaigns' ] ) && isset( $_POST[ 'wpcrm_system_import_campaigns_nonce' ] ) ){
	if( wp_verify_nonce( $_POST[ 'wpcrm_system_import_campaigns_nonce' ], 'wpcrm-system-import-campaigns-nonce' ) ){
		...
		if( !in_array( $file_type, $csv_types ) ){
			$errors[] = __( 'File not allowed, please use a CSV file.', 'wp-crm-system-import-campaigns' );
		}
		if( $file_size > return_bytes( ini_get( 'upload_max_filesize' ) ) ){
			$errors[] = __( 'File size must be less than', 'wp-crm-system-import-campaigns' ) . ini_get( 'upload_max_filesize' );
		}				
		if( empty( $errors ) == true ){
					...
					if( null == get_page_by_title( $title, OBJECT, 'wpcrm-campaign' ) ) {
						...
						if( $campaignCategories != '' ) {
							...
							if ( is_wp_error( $campaignTypes ) ) {
								$error[] = __( 'There was an error with the categories and they could not be set.', 'wp-crm-system-import-campaigns' );
							}
						}
						if( $custom_fields ){
			...
			<div id="message" class="updated">
				<p><strong><?php _e( 'Campaigns uploaded. ', 'wp-crm-system-import-campaigns' ); echo $count_added; _e( ' added. ', 'wp-crm-system-import-campaigns' ); echo $count_skipped; _e( ' skipped.', 'wp-crm-system-import-campaigns' ); ?> </strong></p>
			</div>
		<?php } else { ?>
...
<div class="wrap">
	<div>
		<h2><?php _e( 'Import Campaigns From CSV', 'wp-crm-system-import-campaigns' ); ?></h2>
				...
				<form id="wpcrm_import_campaigns" name="wpcrm_import_campaigns" method="post" action="" enctype="multipart/form-data">
					<tr>
						...
						<td>
							...
							$link = sprintf( wp_kses( __( 'Important: Please make sure your CSV file is in the <a href="%s">correct format</a>. If it is out of order, your fields will not be imported correctly. ', 'wp-crm-system-import-campaigns' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( $url ) );
							echo $link; ?>
						</td>
					</tr>
				</form>
				<form id="wpcrm_export_campaigns" name="wpcrm_export_campaigns" method="post" action="">
					<tr>
						<td>
							...
							<input type="submit" name="export_campaigns" value="<?php _e('Export Campaigns', 'wp-crm-system-import-campaigns' ); ?>" />
						</td>
						<td></td>
					</tr>
				</form>
